...
namespace Queueing
{
    public class Simulation
    {
        ...
        private const float ArrivalTime = 0.0100f,    // 20 millis
                            ProcessingTime = 0.0015f; // 15 millis
        public Simulation()
        {
            for (int i = 0; i < 1000; i++)
            {
                FutureEvents.Add(new Event(EventType.HttpRequest, (Ln(ArrivalTime) * i) + Ln(ProcessingTime), 0, "Http Request is being processed currently.", null));
            }
        }
